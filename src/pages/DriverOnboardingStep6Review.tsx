...
} from "./types/DriverOnBoardingStep2Vehicle.type";
import {
  DRIVER_STEP3_KEY,
...
const createDriver = async (payload: ICreateDriver) => {
  const res = await fetch(`http://localhost:3002/users/drivers`, {
    method: "POST",
    headers: {
...
export default function DriverOnboardingReview() {
  ...
  const handleCreateDriverSubmit = async (data: ICreateDriver) => {
    try {
      await createDriver({
        ...data,
      });
