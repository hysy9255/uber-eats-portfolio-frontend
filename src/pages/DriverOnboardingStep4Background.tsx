...
import {
  DRIVER_STEP4_KEY,
  DriverStep4RightPanel,
  type IDriverOnBoardingStep4Form,
} from "./types/DriverOnBoardingStep4Background.type";

export default function DriverOnboardingBackground() {
  const navigate = useNavigate();

  const defaultValues = loadDraft<IDriverOnBoardingStep4Form>(
    DRIVER_STEP4_KEY,
    {
      legalFullName: "",
      last4SSN: "",
  ...
  const {
    ...
    handleSubmit,
    formState: { errors },
  } = useForm<IDriverOnBoardingStep4Form>({
    mode: "onSubmit",
    defaultValues,
  });
  ...
  const onContinue = (data: IDriverOnBoardingStep4Form) => {
